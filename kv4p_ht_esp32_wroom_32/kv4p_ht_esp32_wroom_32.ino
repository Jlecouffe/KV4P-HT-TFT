...
// Moniteur autonome : au repos (pas de PTT), on reste en réception en permanence
// pour que l'affichage (fréquence + S-mètre) vive sans app connectée.
// Comportement d'origine (muet au repos) :
...
void setup() {
  ...
  delay(100);
  Serial.println();
  Serial.println("===== KV4P + TFT START =====");

  boardSetup();
  Serial.println("BOARD OK");

  displaySetup();
  Serial.println("TFT SETUP OK");
  ...
  Serial.println(radioModuleStatus == RADIO_MODULE_FOUND ? "FOUND" : "NOT FOUND");
...
void loop() {
  ...
  rssiLoop();
  deviceStateLoop();
  uiState.freqMhz   = desiredState.freq_rx;
  uiState.tx        = (mode == MODE_TX);
  uiState.stopped   = (mode == MODE_STOPPED);
  uiState.highPower = (desiredState.flags & HOST_STATE_HIGH_POWER);
  uiState.rssi      = latestRssi;
  uiState.volume    = hw.volume;
  uiState.squelch   = desiredState.squelch;
  uiState.squelched = squelched;
  displayLoop();
}
